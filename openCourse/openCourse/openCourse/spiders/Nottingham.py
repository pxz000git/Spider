...
class CourseSpider(CrawlSpider):
    name = "Nottingham"
    allowed_domains = ["rdmc.nottingham.ac.uk"]
    start_urls = (
        'https://rdmc.nottingham.ac.uk/browse?type=title',

    )
    rules = [
        Rule(LinkExtractor(allow='/handle/internal/(.*?)',
                           restrict_xpaths='//div[@class="artifact-description"]/h4/a'),
             callback='parse_item',
             follow=False),
        Rule(LinkExtractor(allow='browse',
                           restrict_xpaths='//li[@class="next pull-right"]'),
             callback='parse_page',
             follow=True)
    ]
    #每次开始执行抓取，都将之前的数据清空
    ...
    def parse_item(self, response):
        ...
        item = OpencourseItem()
        div_main = sel.xpath("//div[@class='simple-item-view-description item-page-field-wrapper table']")
        if len(div_main) > 0:
            div_main = div_main[0]
