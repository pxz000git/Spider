...
class CourseSpider(CrawlSpider):
    ...
    todayDay = '0'+str(todayDate.day) if todayDate.day < 10 else str(todayDate.day)
    todayMonth = '0'+str(todayDate.month) if todayDate.month < 10 else str(todayDate.month)
    todayLastYear = str(todayDate.year)[-2:0]

    catch_date = ''+todayLastYear+'/'+todayMonth+todayDay+'/'


    rules = [
        Rule(LinkExtractor(allow= catch_date + '(.*?)/(.*?)'),
             callback='parse_item',
             follow=False)
    ]
    #每次开始执行抓取，都将之前的数据清空
